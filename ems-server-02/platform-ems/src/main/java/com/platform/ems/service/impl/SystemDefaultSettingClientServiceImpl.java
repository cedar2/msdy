...
public class SystemDefaultSettingClientServiceImpl extends ServiceImpl<SysDefaultSettingClientMapper, SysDefaultSettingClient> implements ISystemDefaultSettingClientService {
    @Autowired
    ...
    public void setProductSeason(SysDefaultSettingClient client) {
        if (client != null) {
            ...
            //人事管理通知人员（用户账号）
            if (client.getPersonnelMgtNoticeAccountList() != null && client.getPersonnelMgtNoticeAccountList().length > 0) {
                //将获取到的列表用';'分割然后组装成一个String
                String personnelMgtNoticeAccountList = StringUtils.join(client.getPersonnelMgtNoticeAccountList(), ";");
                client.setPersonnelMgtNoticeAccount(personnelMgtNoticeAccountList);
            } else {
                client.setPersonnelMgtNoticeAccount(null);
            }
    ...
    public void getProductSeason(SysDefaultSettingClient client) {
        if (client != null) {
            ...
            //人事管理通知人员
            if (StrUtil.isNotBlank(client.getPersonnelMgtNoticeAccount())) {
                String[] personnelMgtNoticeAccountList = client.getPersonnelMgtNoticeAccount().split(";");
                client.setPersonnelMgtNoticeAccountList(personnelMgtNoticeAccountList);
            }
            //电签超量提醒人员
