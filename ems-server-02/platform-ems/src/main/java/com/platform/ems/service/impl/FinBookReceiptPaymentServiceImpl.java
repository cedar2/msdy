...
public class FinBookReceiptPaymentServiceImpl extends ServiceImpl<FinBookReceiptPaymentMapper,FinBookReceiptPayment>  implements IFinBookReceiptPaymentService {
    @Autowired
    ...
    @Autowired
    private FinBookReceiptPaymentItemMapper itemMapper;
    @Autowired
    private FinBookReceiptPaymentAttachmentMapper atmMapper;
    @Autowired
    ...
    public int insertFinBookReceiptPayment(FinBookReceiptPayment payment) {
        int row = finBookReceiptPaymentMapper.insert(payment);
        if (row > 0) {
            // 写入明细
            if (CollectionUtil.isNotEmpty(payment.getItemList())) {
                bookReceiptPaymentItemService.insertByList(payment);
            }
            //插入日志
            List<OperMsg> msgList = new ArrayList<>();
            msgList = BeanUtils.eq(new FinBookReceiptPayment(), payment);
            MongodbDeal.insert(payment.getBookReceiptPaymentSid(), payment.getHandleStatus(), msgList, TITLE, null);
        }
        return row;
    }
     ...
     */
    public void insertBook(FinSaleInvoice finSaleInvoice) {
    	FinBookReceiptPayment book = new FinBookReceiptPayment();
    	Long sid = IdWorker.getId();
    	book.setBookReceiptPaymentSid(sid);
    	book.setReferDocCategory(finSaleInvoice.getInvoiceCategory());
    	book.setCustomerSid(finSaleInvoice.getCustomerSid());
    	book.setCompanySid(finSaleInvoice.getCompanySid());
    	book.setDocumentDate(finSaleInvoice.getDocumentDate());
    	Calendar cal = Calendar.getInstance();
    	int month = cal.get(Calendar.MONTH) + 1;
    	int year = cal.get(Calendar.YEAR);
    	book.setPaymentYear(Long.parseLong(String.valueOf(year)));
    	book.setPaymentMonth(month);
    	book.setCompanyBrandSid(finSaleInvoice.getCompanyBrandSid());
    	book.setCurrency(finSaleInvoice.getCurrency());
    	book.setCurrencyUnit(finSaleInvoice.getCurrencyUnit());
    	book.setReferDocCategory("销售开票");
    	book.setReferDocSid(finSaleInvoice.getSaleInvoiceSid());
    	finBookReceiptPaymentMapper.insert(book);
    	if(CollectionUtils.isNotEmpty(finSaleInvoice.getFinSaleInvoiceItemList())) {
    		finSaleInvoice.getFinSaleInvoiceItemList().forEach(item->{
    			FinBookReceiptPaymentItem bookItem = new  FinBookReceiptPaymentItem();
    			bookItem.setBookReceiptPaymentSid(sid);
    			bookItem.setCurrencyAmountTaxSk(item.getCurrencyAmountTax());
    			bookItem.setTaxRate(item.getTaxRate());
                bookItem.setIsFinanceVerify(ConstantsEms.NO);
    			itemMapper.insert(bookItem);
    		});
    	}
    	if(CollectionUtils.isNotEmpty(finSaleInvoice.getAttachmentList())) {
    		finSaleInvoice.getAttachmentList().forEach(atm->{
    			FinBookReceiptPaymentAttachment bookAtm = new FinBookReceiptPaymentAttachment();
    			bookAtm.setBookReceiptPaymentSid(sid);
    			bookAtm.setBeginTime(atm.getBeginTime());
    			bookAtm.setEndTime(atm.getEndTime());
    			bookAtm.setFileName(atm.getFileName());
    			bookAtm.setFilePath(atm.getFilePath());
    			bookAtm.setFileType(atm.getFileType());
    			atmMapper.insert(bookAtm);
    		});
    	}
    }

    //流水报表查询
    public List<FinBookReceiptPayment> getReportForm(FinBookReceiptPayment entity){
        List<FinBookReceiptPayment> responseList = finBookReceiptPaymentMapper.getReportForm(entity);
        return responseList;
    }
    ...
    public AjaxResult importData(MultipartFile file) {
        List<FinBookReceiptPayment> responseList = new ArrayList<>();
        //错误信息
        ...
        try {
            ...
            for (int i = 0; i < readAll.size(); i++) {
                 ...
                 * 已收金额,核销中金额,已核销金额
                 */
                String yis = objects.get(3) == null || objects.get(3) == "" ? null : objects.get(3).toString();
                BigDecimal currencyAmountTaxSk = null;
                ...
                if (StrUtil.isBlank(yis)) {
                    errMsg = new CommonErrMsgResponse();
                    errMsg.setItemNum(num);
                    errMsg.setMsg("已收金额不可为空，导入失败！");
                    errMsgList.add(errMsg);
                }else {
                    if (!JudgeFormat.isValidDouble(yis,10,5)){
                        errMsg = new CommonErrMsgResponse();
                        errMsg.setItemNum(num);
                        errMsg.setMsg("已收金额格式错误，导入失败！");
                        errMsgList.add(errMsg);
                    }else {
                        currencyAmountTaxSk = new BigDecimal(yis);
                        if (BigDecimal.ZERO.compareTo(currencyAmountTaxSk) >= 0){
                            errMsg = new CommonErrMsgResponse();
                            errMsg.setItemNum(num);
                            errMsg.setMsg("已收金额不能小于等于0，导入失败！");
                            errMsgList.add(errMsg);
                        }
                    }
                }
                if (StrUtil.isBlank(yhx)) {
                    ...
                    if (!JudgeFormat.isValidDouble(yhx,10,5)){
                        ...
                        if (BigDecimal.ZERO.compareTo(currencyAmountTaxYhx) > 0){
                            ...
                            if (currencyAmountTaxSk != null) {
                                if (currencyAmountTaxSk.compareTo(currencyAmountTaxYhx) < 0) {
                                    errMsg = new CommonErrMsgResponse();
                                    errMsg.setItemNum(num);
                                    errMsg.setMsg("已核销金额不能大于已收款金额，导入失败！");
                                    errMsgList.add(errMsg);
                                }
                            }
                        }
                ...
                String bookSourceCategory = ConstantsFinance.BOOK_SOURCE_CAT_YSK;
                try {
                ...
                if (documentDate != null && customerSid != null && companySid != null) {
                    String key = documentDateS + "-" + customerSid + "-" + companySid;
                    // 判断是否与表格内的编码重复
                    if (uniqueMap.get(key) == null) {
                        ...
                        errMsg = new CommonErrMsgResponse();
                        errMsg.setItemNum(num);
                        errMsg.setMsg("表格中，组合“凭证日期+客户+公司”已存在，导入失败！");
                        errMsgList.add(errMsg);
                    }
                }
                ...
                if (CollectionUtils.isEmpty(errMsgList)){
                    ...
                    finBookReceiptPayment.setCompanyName(companyName).setCompanyCode(companyCode)
                            .setCompanyShortName(companyShortName).setCustomerCode(customerCode)
                            .setCustomerName(customerName).setCustomerShortName(customerShortName)
                            .setCustomerSid(customerSid).setCompanySid(companySid).setDocumentDate(documentDate)
                            .setBookType(ConstantsFinance.BOOK_TYPE_SK)
                    ...
                    finBookReceiptPayment.setCurrencyAmountTaxSk(currencyAmountTaxSk)
                            .setClearStatus(clearStatus)
                            .setCurrencyAmountTaxHxz(currencyAmountTaxHxz).setCurrencyAmountTaxYhx(currencyAmountTaxYhx)
                            .setTaxRate(taxRateValue).setRemark(remark);
                    LocalDate localDate = documentDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
                    finBookReceiptPayment.setPaymentYear((long) localDate.getYear())
                            .setPaymentMonth(localDate.getMonthValue());
                    responseList.add(finBookReceiptPayment);
                }
            }
    ...
    @Override
    public int addForm(List<FinBookReceiptPayment> request) {
        if (CollectionUtils.isEmpty(request)) {
            return 0;
        }
        request.forEach(item -> {
            finBookReceiptPaymentMapper.insert(item);
            //预付明细表
            FinBookReceiptPaymentItem finBookReceiptPaymentItem = new FinBookReceiptPaymentItem();
            finBookReceiptPaymentItem.setCurrencyAmountTaxSk(item.getCurrencyAmountTaxSk())
                    .setCurrencyAmountTaxHxz(item.getCurrencyAmountTaxHxz())
                    .setCurrencyAmountTaxYhx(item.getCurrencyAmountTaxYhx());
            if (BigDecimal.ZERO.compareTo(item.getCurrencyAmountTaxSk()) < 0) {
                finBookReceiptPaymentItem.setCurrencyAmountTaxSk(item.getCurrencyAmountTaxSk());
            }
            if (BigDecimal.ZERO.compareTo(item.getCurrencyAmountTaxHxz()) < 0) {
                finBookReceiptPaymentItem.setCurrencyAmountTaxHxz(item.getCurrencyAmountTaxHxz());
            }
            if (BigDecimal.ZERO.compareTo(item.getCurrencyAmountTaxYhx()) < 0) {
                finBookReceiptPaymentItem.setCurrencyAmountTaxYhx(item.getCurrencyAmountTaxYhx());
            }
            BigDecimal taxRate = null;
            if (StrUtil.isNotBlank(item.getTaxRate())){
                taxRate = new BigDecimal(item.getTaxRate());
            }
            finBookReceiptPaymentItem
                    .setBookReceiptPaymentSid(item.getBookReceiptPaymentSid())
                    .setItemNum((long)1)
                    .setClearStatus(item.getClearStatus())
                    .setTaxRate(taxRate).setRemark(item.getRemark());
            finBookReceiptPaymentItem.setIsFinanceVerify(ConstantsEms.NO);
            itemMapper.insert(finBookReceiptPaymentItem);
        });
        return request.size();
    }
