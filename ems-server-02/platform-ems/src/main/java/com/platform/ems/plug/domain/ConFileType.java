...
@ApiModel
@TableName(value = "s_con_file_type")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConFileType extends EmsBaseEntity {
     ...
     * 附件类型编码
     */
    @NotBlank(message = "附件类型编码不能为空")
    @Length(max = 8, message = "附件类型编码不能超过8个字符")
    ...
    @Length(max = 300, message = "附件类型名称不能超过300个字符")
    ...
    @Excel(name = "数据对象类别")
    @TableField(exist = false)
    @ApiModelProperty(value = "数据对象类别name")
    private String dataobjectCategoryName;

    /**
     * 是否允许上传多个附件（数据字典的键值或配置档案的编码）
     */
    @Excel(name = "是否允许上传多个附件", dictType = "sys_yes_no")
    @ApiModelProperty(value = "是否允许上传多个附件（数据字典的键值或配置档案的编码）")
    private String isUploadMultifile;
