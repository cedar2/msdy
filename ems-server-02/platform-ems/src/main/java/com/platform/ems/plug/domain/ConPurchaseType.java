...
@ApiModel
@TableName(value = "s_con_purchase_type")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConPurchaseType extends EmsBaseEntity {
    /**
     * 租户ID
     */
    ...
    @ApiModelProperty(value = "系统SID-采购类型sid")
    private Long sid;
     ...
     * 采购类型编码
     */
    @NotBlank(message = "采购类型编码不能为空")
    @Length(max = 8, message = "采购类型编码不能超过8个字符")
    @Excel(name = "采购类型编码")
    ...
    @Length(max = 300, message = "采购类型名称不能超过300个字符")
    @Excel(name = "采购类型名称")
    @ApiModelProperty(value = "采购类型名称")
    private String name;

    /**
     * 租户类型（数据字典的键值或配置档案的编码）
     */
    @Excel(name = "租户类型", dictType = "s_client_type")
    @ApiModelProperty(value = "租户类型（数据字典的键值或配置档案的编码）")
