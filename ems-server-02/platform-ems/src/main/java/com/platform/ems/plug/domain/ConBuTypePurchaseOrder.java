...
/**
 * 业务类型_采购订单对象 s_con_bu_type_purchase_order
 *
 * @author chenkw
 * @date 2021-05-20
 */
...
@ApiModel
@TableName(value = "s_con_bu_type_purchase_order")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConBuTypePurchaseOrder extends EmsBaseEntity {
    ...
    @ApiModelProperty(value = "系统SID-业务类型(采购订单)sid")
    private Long sid;
     ...
     * 业务类型(采购订单)编码
     */
    @NotEmpty(message = "业务类型编码不能为空")
    @Length(max = 8, message = "业务类型编码不能超过8个字符")
    @Excel(name = "业务类型编码")
    @ApiModelProperty(value = "业务类型(采购订单)编码")
    private String code;
    ...
    @Excel(name = "业务类型名称")
    @ApiModelProperty(value = "业务类型(采购订单)名称")
    private String name;

    /**
     * 采购交货类型（数据字典的键值或配置档案的编码）
     */
    @Excel(name = "采购交货类型", dictType = "s_po_delivery_type")
    @ApiModelProperty(value = "采购交货类型（数据字典的键值或配置档案的编码）")
    ...
    @Excel(name = "是否一次性采购", dictType = "sys_yes_no")
    @ApiModelProperty(value = "是否一次性采购（数据字典的键值或配置档案的编码）")
