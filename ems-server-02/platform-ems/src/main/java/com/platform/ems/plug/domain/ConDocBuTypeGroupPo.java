...
/**
 * 采购订单单据类型与业务类型组合关系对象 s_con_doc_bu_type_group_po
 *
 * @author chenkw
 * @date 2021-12-24
 */
...
@ApiModel
@TableName(value = "s_con_doc_bu_type_group_po")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConDocBuTypeGroupPo extends EmsBaseEntity {
    ...
    /**
     * 系统SID-采购订单单据类型与业务类型组合关系信息
     */
    @TableId
    @JsonSerialize(using = ToStringSerializer.class)
    @ApiModelProperty(value = "系统SID-采购订单单据类型与业务类型组合关系信息")
    private Long sid;
    ...
    /**
     * 单据类型(采购订单)编码（数据字典的键值或配置档案的编码）单据类型(采购订单)编码（数据字典的键值或配置档案的编码）
     */
    @Excel(name = "单据类型编码")
    @NotBlank(message = "请选择单据类型")
    @ApiModelProperty(value = "单据类型(采购订单)编码（数据字典的键值或配置档案的编码）")
    ...
    /**
     * 业务类型(采购订单)编码（数据字典的键值或配置档案的编码）
     */
    @Excel(name = "业务类型编码")
    @NotBlank(message = "请选择业务类型")
    @ApiModelProperty(value = "业务类型(采购订单)编码（数据字典的键值或配置档案的编码）")
    ...
    @TableField(exist = false)
    @ApiModelProperty(value = "单据类型(采购订单)多选查询")
    private String[] docTypeCodeList;

    @TableField(exist = false)
    @ApiModelProperty(value = "业务类型(采购订单)多选查询")
