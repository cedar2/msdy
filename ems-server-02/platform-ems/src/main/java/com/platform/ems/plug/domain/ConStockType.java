...
@ApiModel
@TableName(value = "s_con_stock_type")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConStockType extends EmsBaseEntity {
    ...
    @ApiModelProperty(value = "系统SID-库存类型sid")
    private Long sid;
     ...
     * 库存类型编码
     */
    @Length(max = 8, message = "库存类型编码不能超过8个字符")
    @NotBlank(message = "库存类型编码不能为空")
    @Excel(name = "库存类型编码")
     ...
     * 库存类型名称
     */
    @Length(max = 300, message = "库存类型名称不能超过300个字符")
    @NotBlank(message = "库存类型名称不能为空")
    @Excel(name = "库存类型名称")
