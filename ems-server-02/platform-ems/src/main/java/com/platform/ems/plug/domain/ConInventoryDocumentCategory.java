...
/**
 * 库存凭证类别对象 s_con_inventory_document_category
 *
 * @author c
 * @date 2021-07-29
 */
...
@ApiModel
@TableName(value = "s_con_inventory_document_category")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConInventoryDocumentCategory extends EmsBaseEntity {
     ...
     * 库存凭证类别编码
     */
    @NotBlank(message = "库存凭证类别编码不能为空")
    @Length(max = 8, message = "库存凭证类别编码不能超过8个字符")
    @Excel(name = "库存凭证类别编码")
     ...
     * 库存凭证类别名称
     */
    @NotBlank(message = "库存凭证类别名称不能为空")
    @Length(max = 300, message = "库存凭证类别名称不能超过300个字符")
    @Excel(name = "库存凭证类别名称")
