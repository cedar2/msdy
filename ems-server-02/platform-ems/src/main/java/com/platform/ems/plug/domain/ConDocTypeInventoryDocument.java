...
/**
 * 单据类型_库存凭证对象 s_con_doc_type_inventory_document
 *
 * @author linhongwei
 * @date 2021-09-17
 */
...
@ApiModel
@TableName(value = "s_con_doc_type_inventory_document")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConDocTypeInventoryDocument extends EmsBaseEntity {
    ...
    @ApiModelProperty(value = "系统SID-单据类型(库存凭证)sid")
    private Long sid;
    ...
    @NotEmpty(message = "单据类型编码不能为空")
    @Excel(name = "单据类型编码")
    @ApiModelProperty(value = "单据类型(库存凭证)编码")
    private String code;
    ...
    @NotEmpty(message = "单据类型名称不能为空")
    @Excel(name = "单据类型名称")
    @ApiModelProperty(value = "单据类型(库存凭证)名称")
    private String name;
