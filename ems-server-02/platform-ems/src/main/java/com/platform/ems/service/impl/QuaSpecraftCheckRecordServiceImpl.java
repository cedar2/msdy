...
public class QuaSpecraftCheckRecordServiceImpl extends ServiceImpl<QuaSpecraftCheckRecordMapper, QuaSpecraftCheckRecord> implements IQuaSpecraftCheckRecordService {
    @Autowired
    private QuaSpecraftCheckRecordMapper quaSpecraftCheckRecordMapper;
    @Autowired
    private QuaSpecraftCheckRecordAttachMapper attachMapper;
    @Autowired
    ...
    private static final String TITLE = "特殊工艺检测问题台账";

    /**
     * 查询特殊工艺检测问题台账
     *
     * @param specraftCheckRecordSid 特殊工艺检测问题台账ID
     * @return 特殊工艺检测问题台账
     */
    @Override
    public QuaSpecraftCheckRecord selectQuaSpecraftCheckRecordById(Long specraftCheckRecordSid) {
        QuaSpecraftCheckRecord quaSpecraftCheckRecord = quaSpecraftCheckRecordMapper.selectQuaSpecraftCheckRecordById(specraftCheckRecordSid);
        // 特殊字段处理
        getData(quaSpecraftCheckRecord);
        // 附件
        List<QuaSpecraftCheckRecordAttach> attachList = attachMapper.selectQuaSpecraftCheckRecordAttachList(new QuaSpecraftCheckRecordAttach()
                .setSpecraftCheckRecordSid(specraftCheckRecordSid));
        quaSpecraftCheckRecord.setAttachmentList(new ArrayList<>());
        if (CollectionUtil.isNotEmpty(attachList)) {
            quaSpecraftCheckRecord.setAttachmentList(attachList);
        }
        // 操作日志
        MongodbUtil.find(quaSpecraftCheckRecord);
        return quaSpecraftCheckRecord;
    }

    /**
     * 写入数据字段处理
     */
    private void setData(QuaSpecraftCheckRecord record) {
        // 图片
        String picture = null;
     ...
     * 读取数据字段处理
     */
    private void getData(QuaSpecraftCheckRecord record) {
        if (record == null) {
            return;
        }
        // 图片
     ...
     * 查询特殊工艺检测问题台账列表
     *
     * @param quaSpecraftCheckRecord 特殊工艺检测问题台账
     * @return 特殊工艺检测问题台账
     */
    @Override
    public List<QuaSpecraftCheckRecord> selectQuaSpecraftCheckRecordList(QuaSpecraftCheckRecord quaSpecraftCheckRecord) {
        List<QuaSpecraftCheckRecord> recordList = quaSpecraftCheckRecordMapper.selectQuaSpecraftCheckRecordList(quaSpecraftCheckRecord);
        // 图片视频字段查询页面要
        if (CollectionUtil.isNotEmpty(recordList)) {
            for (QuaSpecraftCheckRecord record : recordList) {
                getData(record);
            }
        }
        return recordList;
    ...
    public int insertQuaSpecraftCheckRecord(QuaSpecraftCheckRecord quaSpecraftCheckRecord) {
        // 写入确认人
        if (ConstantsEms.CHECK_STATUS.equals(quaSpecraftCheckRecord.getHandleStatus())) {
            quaSpecraftCheckRecord.setConfirmDate(new Date()).setConfirmerAccount(ApiThreadLocalUtil.get().getUsername());
        }
        // 特殊字段处理
        setData(quaSpecraftCheckRecord);
        int row = quaSpecraftCheckRecordMapper.insert(quaSpecraftCheckRecord);
        if (row > 0) {
            // 附件清单
            if (CollectionUtil.isNotEmpty(quaSpecraftCheckRecord.getAttachmentList())) {
                quaSpecraftCheckRecord.getAttachmentList().forEach(item->{
                    item.setSpecraftCheckRecordSid(quaSpecraftCheckRecord.getSpecraftCheckRecordSid());
                });
                attachMapper.inserts(quaSpecraftCheckRecord.getAttachmentList());
            }
            //插入日志
            List<OperMsg> msgList = new ArrayList<>();
            msgList = BeanUtils.eq(new QuaSpecraftCheckRecord(), quaSpecraftCheckRecord);
            MongodbDeal.insert(quaSpecraftCheckRecord.getSpecraftCheckRecordSid(), quaSpecraftCheckRecord.getHandleStatus(), msgList, TITLE, null, BusinessType.IMPORT.getValue());
        }
        return row;
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void updateAttach(QuaSpecraftCheckRecord record) {
        // 先删后加
        attachMapper.delete(new QueryWrapper<QuaSpecraftCheckRecordAttach>().lambda()
                .eq(QuaSpecraftCheckRecordAttach::getSpecraftCheckRecordSid, record.getSpecraftCheckRecordSid()));
        if (CollectionUtil.isNotEmpty(record.getAttachmentList())) {
            record.getAttachmentList().forEach(att -> {
                // 如果是新的
                if (att.getSpecraftCheckRecordAttachSid() == null) {
                    att.setSpecraftCheckRecordSid(record.getSpecraftCheckRecordSid());
                }
                // 如果是旧的就写入更改日期
                else {
    ...
    public int updateQuaSpecraftCheckRecord(QuaSpecraftCheckRecord quaSpecraftCheckRecord) {
        QuaSpecraftCheckRecord original = quaSpecraftCheckRecordMapper.selectQuaSpecraftCheckRecordById(quaSpecraftCheckRecord.getSpecraftCheckRecordSid());
        // 写入确认人
        if (ConstantsEms.CHECK_STATUS.equals(quaSpecraftCheckRecord.getHandleStatus())) {
            quaSpecraftCheckRecord.setConfirmDate(new Date()).setConfirmerAccount(ApiThreadLocalUtil.get().getUsername());
        }
        ...
        msgList = BeanUtils.eq(original, quaSpecraftCheckRecord);
        if (CollectionUtil.isNotEmpty(msgList)) {
            quaSpecraftCheckRecord.setUpdateDate(new Date()).setUpdaterAccount(ApiThreadLocalUtil.get().getUsername());
        }
        int row = quaSpecraftCheckRecordMapper.updateAllById(quaSpecraftCheckRecord);
        if (row > 0) {
            // 附件清单
            updateAttach(quaSpecraftCheckRecord);
            //插入日志
            MongodbDeal.update(quaSpecraftCheckRecord.getSpecraftCheckRecordSid(), original.getHandleStatus(),
                    quaSpecraftCheckRecord.getHandleStatus(), msgList, TITLE, null);
        }
        return row;
    }
    ...
    public int changeQuaSpecraftCheckRecord(QuaSpecraftCheckRecord quaSpecraftCheckRecord) {
        QuaSpecraftCheckRecord response = quaSpecraftCheckRecordMapper.selectQuaSpecraftCheckRecordById(quaSpecraftCheckRecord.getSpecraftCheckRecordSid());
        // 特殊字段处理
        setData(quaSpecraftCheckRecord);
        // 更新人更新日期
        List<OperMsg> msgList;
        msgList = BeanUtils.eq(response, quaSpecraftCheckRecord);
        if (CollectionUtil.isNotEmpty(msgList)) {
            quaSpecraftCheckRecord.setUpdateDate(new Date()).setUpdaterAccount(ApiThreadLocalUtil.get().getUsername());
        }
        int row = quaSpecraftCheckRecordMapper.updateAllById(quaSpecraftCheckRecord);
        if (row > 0) {
            ...
            updateAttach(quaSpecraftCheckRecord);
            //插入日志
            MongodbUtil.insertUserLog(quaSpecraftCheckRecord.getSpecraftCheckRecordSid(), BusinessType.CHANGE.getValue(), response, quaSpecraftCheckRecord, TITLE);
        }
        return row;
    }
     ...
     * 批量删除特殊工艺检测问题台账
     *
     * @param specraftCheckRecordSids 需要删除的特殊工艺检测问题台账ID
     * @return 结果
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int deleteQuaSpecraftCheckRecordByIds(List<Long> specraftCheckRecordSids) {
        List<QuaSpecraftCheckRecord> list = quaSpecraftCheckRecordMapper.selectList(new QueryWrapper<QuaSpecraftCheckRecord>()
                .lambda().in(QuaSpecraftCheckRecord::getSpecraftCheckRecordSid, specraftCheckRecordSids));
        int row = quaSpecraftCheckRecordMapper.deleteBatchIds(specraftCheckRecordSids);
        if (row > 0) {
            // 附件
            attachMapper.delete(new QueryWrapper<QuaSpecraftCheckRecordAttach>()
                    .lambda().in(QuaSpecraftCheckRecordAttach::getSpecraftCheckRecordSid, specraftCheckRecordSids));
            // 操作日志
            list.forEach(o -> {
                List<OperMsg> msgList = new ArrayList<>();
                msgList = BeanUtils.eq(o, new QuaSpecraftCheckRecord());
                MongodbUtil.insertUserLog(o.getSpecraftCheckRecordSid(), BusinessType.DELETE.getValue(), msgList, TITLE);
            });
        }
        return row;
    }
     ...
     * 更改确认状态
     *
     * @param quaSpecraftCheckRecord
     * @return
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int check(QuaSpecraftCheckRecord quaSpecraftCheckRecord) {
        Long[] sids = quaSpecraftCheckRecord.getSpecraftCheckRecordSidList();
        if (ArrayUtil.isEmpty(sids)) {
            return 0;
        }
        LambdaUpdateWrapper<QuaSpecraftCheckRecord> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.in(QuaSpecraftCheckRecord::getSpecraftCheckRecordSid, sids);
        updateWrapper.set(QuaSpecraftCheckRecord::getHandleStatus, quaSpecraftCheckRecord.getHandleStatus());
        if (ConstantsEms.CHECK_STATUS.equals(quaSpecraftCheckRecord.getHandleStatus())) {
            updateWrapper.set(QuaSpecraftCheckRecord::getConfirmDate, new Date());
            updateWrapper.set(QuaSpecraftCheckRecord::getConfirmerAccount, ApiThreadLocalUtil.get().getUsername());
        }
        int row = quaSpecraftCheckRecordMapper.update(null, updateWrapper);
        if (row > 0) {
            for (Long id : sids) {
                //插入日志
                MongodbDeal.check(id, quaSpecraftCheckRecord.getHandleStatus(), null, TITLE, null);
            }
        }
        return row;
    }
    ...
    public EmsResultEntity importRecord(MultipartFile file) {
        ...
        try {
            ...
            // 数据字典特殊工艺类型
            List<DictData> specraftTypeList = sysDictDataService.selectDictData("s_specraft_type");
            specraftTypeList = specraftTypeList.stream().filter(o -> o.getStatus().equals(ConstantsEms.SYS_COMMON_STATUS_Y)).collect(Collectors.toList());
            Map<String, String> specraftTypeMaps = specraftTypeList.stream().collect(Collectors.toMap(DictData::getDictLabel, DictData::getDictValue, (key1, key2) -> key2));
            ...
            // 列表
            List<QuaSpecraftCheckRecord> recordList = new ArrayList<>();
            ...
            for (int i = 0; i < readAll.size(); i++) {
                ...
                // 主表
                QuaSpecraftCheckRecord record = new QuaSpecraftCheckRecord();
                /*
                 * 商品编码 必填
                 */
                 ...
                 * 特殊工艺类型 必填
                 */
                String specraftTypeName = objects.get(2) == null || objects.get(2) == "" ? null : objects.get(2).toString().trim();
                /*
                 * 检测项名称 配置档案 必填
                 */
                String checkItemName = objects.get(3) == null || objects.get(3) == "" ? null : objects.get(3).toString().trim();
                /*
                 * 判定结果 数据字典 必填
                 */
                 ...
                 * 检测结果值 必填
                 */
                String checkResultValue = objects.get(5) == null || objects.get(5) == "" ? null : objects.get(5).toString().trim();
                /*
                 * 样品类别 字典 必填
                 */
                String sampleCategoryName = objects.get(6) == null || objects.get(6) == "" ? null : objects.get(6).toString().trim();
                /*
                 * 样品数量 必填
                 */
                String quantityS = objects.get(7) == null || objects.get(7) == "" ? null : objects.get(7).toString().trim();
                /*
                 * 问题类型 字典 必填
                 */
                String defectTypeName = objects.get(8) == null || objects.get(8) == "" ? null : objects.get(8).toString().trim();
                /*
                 * 问题描述 必填
                 */
                String defectDescription = objects.get(9) == null || objects.get(9) == "" ? null : objects.get(9).toString().trim();
                /*
                 * 解决状态 字典 必填
                 */
                String resolveStatusName = objects.get(10) == null || objects.get(10) == "" ? null : objects.get(10).toString().trim();
                /*
                 * 解决说明 选填
                 */
                String solutionRemark = objects.get(11) == null || objects.get(11) == "" ? null : objects.get(11).toString().trim();
                /*
                 * 检测参考值 选填
                 */
                String checkReferenceValue = objects.get(12) == null || objects.get(12) == "" ? null : objects.get(12).toString().trim();
                /*
                 * 产品季名称 选填
                 */
                String productSeasonName = objects.get(13) == null || objects.get(13) == "" ? null : objects.get(13).toString().trim();
                /*
                 * 工厂简称 选填
                 */
                String plantShortName = objects.get(14) == null || objects.get(14) == "" ? null : objects.get(14).toString().trim();
                /*
                 * 检测类型 字典 选填
                 */
                String checkTypeName = objects.get(15) == null || objects.get(15) == "" ? null : objects.get(15).toString().trim();
                /*
                 * 检测标准名称 字典 选填
                 */
                String checkStandardName = objects.get(16) == null || objects.get(16) == "" ? null : objects.get(16).toString().trim();
                /*
                 * 检测批次 选填
                 */
                String checkBatchS = objects.get(17) == null || objects.get(17) == "" ? null : objects.get(17).toString().trim();
                /*
                 * 供应商名称 选填
                 */
                String vendorName = objects.get(18) == null || objects.get(18) == "" ? null : objects.get(18).toString().trim();
                /*
                 * 报告号 选填
                 */
                String reportCode = objects.get(19) == null || objects.get(19) == "" ? null : objects.get(19).toString().trim();
                /*
                 * 检测方类型 选填
                 */
                String checkPartner = objects.get(20) == null || objects.get(20) == "" ? null : objects.get(20).toString().trim();
                /*
                 * 检测日期 选填
                 */
                String checkDateS = objects.get(21) == null || objects.get(21) == "" ? null : objects.get(21).toString().trim();
                /*
                 * 等级 字典 选填
                 */
                String gradeName = objects.get(22) == null || objects.get(22) == "" ? null : objects.get(22).toString().trim();
                /*
                 * 安全类别 字典 选填
                 */
                String safeCategoryName = objects.get(23) == null || objects.get(23) == "" ? null : objects.get(23).toString().trim();
                /*
                 * 备注 选填
                 */
                String remark = objects.get(24) == null || objects.get(24) == "" ? null : objects.get(24).toString().trim();
                ...
                if (StrUtil.isBlank(materialCode)) {
                    ...
                    try {
                        ...
                        errMsgList.add(errMsg);
                        log.error("物料检测问题台账Service业务层处理->导入功能->商品编码查询报错");
                        e.printStackTrace();
                    }
                }

                // 特殊工艺类型
                String specraftType = null;
                if (StrUtil.isBlank(specraftTypeName)) {
                    errMsg = new CommonErrMsgResponse();
                    errMsg.setItemNum(num);
                    errMsg.setMsg("特殊工艺类型不能为空，导入失败！");
                    errMsgList.add(errMsg);
                } else {
                    // 通过数据字典标签获取数据字典的值
                    specraftType = specraftTypeMaps.get(specraftTypeName);
                    if (StrUtil.isBlank(specraftType)) {
                        errMsg = new CommonErrMsgResponse();
                        errMsg.setItemNum(num);
                        errMsg.setMsg("特殊工艺类型填写错误，导入失败！");
                        errMsgList.add(errMsg);
                    }
                }
                ...
                if (CollectionUtil.isEmpty(errMsgList)) {
                    record.setMaterialCode(materialCode).setMaterialSid(materialSid).setSpecraftType(specraftType)
                            ...
                            .setVendorName(vendorName).setRemark(remark).setReportCode(reportCode)
                            .setCheckPartner(checkPartner).setCheckDate(checkDate).setGrade(grade).setSafeCategory(safeCategory)
                            .setHandleStatus(ConstantsEms.SAVA_STATUS).setCheckCategory("TSGY");
            ...
            if (CollectionUtil.isNotEmpty(errMsgList)) {
                ...
                for (QuaSpecraftCheckRecord record : recordList) {
                    insertQuaSpecraftCheckRecord(record);
                }
                String message = null;
                if (CollectionUtil.isNotEmpty(infoMsgList)) {
                    message = "";
