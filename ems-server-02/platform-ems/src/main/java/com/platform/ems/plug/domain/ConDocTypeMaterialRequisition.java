...
/**
 * 单据类型_领退料单对象 s_con_doc_type_material_requisition
 *
 * @author chenkw
 * @date 2021-05-20
 */
...
@ApiModel
@TableName(value = "s_con_doc_type_material_requisition")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConDocTypeMaterialRequisition extends EmsBaseEntity {
    ...
    @ApiModelProperty(value = "系统SID-单据类型(领退料单)sid")
    private Long sid;
    ...
    @Excel(name = "单据类型编码")
    @ApiModelProperty(value = "单据类型(领退料单)编码")
    private String code;
    ...
    @Excel(name = "单据类型名称")
    @ApiModelProperty(value = "单据类型(领退料单)名称")
    private String name;

    @NotBlank(message = "业务标识不能为空")
    @Excel(name = "业务标识", dictType = "s_business_material_requisition")
    @ApiModelProperty(value = "领退料业务标识（数据字典的键值货配置档案的编码）")
    private String businessFlag;

    @TableField(exist = false)
    @ApiModelProperty(value = "领退料业务标识（多选）")
