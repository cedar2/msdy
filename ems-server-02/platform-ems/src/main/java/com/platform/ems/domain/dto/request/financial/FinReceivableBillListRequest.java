...
/**
 * 收款单列表请求实体 FinReceivableBillListRequest
 *
 * @author chenkaiwen
 * @date 2021-07-26
 */
...
@ApiModel
public class FinReceivableBillListRequest extends EmsBaseEntity implements Serializable {
    ...
    @ApiModelProperty(value = "到账状态（数据字典的键值或配置档案的编码）（单选）")
    private String receiptPaymentStatus;

    @ApiModelProperty(value = "到账状态（数据字典的键值或配置档案的编码）")
    private String[] receiptPaymentStatusList;

    @ApiModelProperty(value = "销售部门（数据字典的键值）（单选）")
    private String saleDepartment;

    @ApiModelProperty(value = "销售部门（数据字典的键值）")
    private String[] saleDepartmentList;

    @ApiModelProperty(value = "销售组织（数据字典的键值）（单选）")
