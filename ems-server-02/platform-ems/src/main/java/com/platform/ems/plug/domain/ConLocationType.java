...
@ApiModel
@TableName(value = "s_con_location_type")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConLocationType extends EmsBaseEntity {
    ...
    @ApiModelProperty(value = "系统SID-库位类型sid")
    private Long sid;
     ...
     * 库位类型编码
     */
    @Length(max = 8, message = "库位类型编码不能超过8个字符")
    @NotBlank(message = "库位类型编码不能为空")
    @Excel(name = "库位类型编码")
     ...
     * 库位类型名称
     */
    @Length(max = 300, message = "库位类型名称不能超过300个字符")
    @NotBlank(message = "库位类型名称不能为空")
    @Excel(name = "库位类型名称")
